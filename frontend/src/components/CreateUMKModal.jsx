...
const CreateUMKModal = ({ open, onClose, onUMKCreated, loading, error, formComponent }) => {
  return (
    <Dialog
      ...
      maxWidth="sm"
      fullWidth
      sx={{ '& .MuiDialog-paper': { borderRadius: '16px', boxShadow: '0px 4px 20px rgba(0, 0, 0, 0.1)' } }}
    >
      ...
      <DialogActions sx={{ justifyContent: 'center', p: 3, gap: 2 }}>
        ...
        >
          {loading ? <CircularProgress size={24} sx={{ color: 'white' }} /> : 'Создать'}
        </Button>
      </DialogActions>
    </Dialog>
  );
